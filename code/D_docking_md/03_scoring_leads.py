...
# Produces: 03_scoring_leads.pdf
# Conda env: dock-md   (run with this environment activated)
# Inputs (expected alongside / in data/): druggable_target_ranking.csv, style.css, master_refs.json, 03_scoring_leads.md
# Source artifact version: 0c6237f2-7419-4865-a989-1e4b9f3522e7
...
fig_paths = {}  # no figure for paper 03_scoring_leads
...
name, html_body = render_paper3("03_scoring_leads.md")
full = f"<html><head><meta charset='utf-8'><style>{CSS}</style></head><body>{html_body}</body></html>"
weasyprint.HTML(string=full, base_url=".").write_pdf("03_scoring_leads.pdf")