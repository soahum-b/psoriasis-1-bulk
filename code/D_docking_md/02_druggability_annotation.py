...
# Produces: 02_druggability_annotation.pdf
# Conda env: dock-md   (run with this environment activated)
# Inputs (expected alongside / in data/): druggability_annotation.csv, style.css, master_refs.json, 02_druggability_annotation.md
# Source artifact version: 9a2b3c90-e287-477e-9236-f7f0fb7eb843
...
# Load figure path for this paper
fig_paths = {}
try:
    r = host.artifacts(filename="fig_druggability_landscape.png", exact=True)
    if r["artifacts"]:
        vid = r["artifacts"][0]["latest_version_id"]
        fig_paths["02_druggability_annotation"] = host.artifact_path(vid)
except Exception:
    pass
...
name, html_body = render_paper3("02_druggability_annotation.md")
full = f"<html><head><meta charset='utf-8'><style>{CSS}</style></head><body>{html_body}</body></html>"
weasyprint.HTML(string=full, base_url=".").write_pdf("02_druggability_annotation.pdf")