...
# Produces: staging_figures.R
# Conda env: psoriasis-r   (run with this environment activated)
# Inputs (expected alongside / in data/): sample_classification.rds, stat3_activity_3group.rds, ssgsea_3group.rds, class_enrichment.rds, gradient_gene_classes.csv, trend_SRP165679.rds, collectri_regulon.rds
# Source artifact version: e594adc9-f3ab-4376-bc03-798a658706de
...
trend <- readRDS("trend_SRP165679.rds")
g     <- fread("gradient_gene_classes.csv")
enr   <- readRDS("class_enrichment.rds")
obj   <- readRDS("ssgsea_3group.rds"); sc<-obj$scores; grp<-obj$grp
s3    <- readRDS("stat3_activity_3group.rds")
...
ggsave("fig_s2_gradient_taxonomy.png", pA+pB+plot_layout(widths=c(1,1.5)),
...
ggsave("fig_s3_pathway_timing.png",
...
ggsave("fig_s4_stat3_il17_on_axis.png",
...
ggsave("fig_s4b_stat3_activity_axis.png",
...
qB<-ggplot(pf,aes(pn_frac))+geom_histogram(bins=40,fill="#DD8452",colour="white",linewidth=.2)+
  ...
  theme_bw(base_size=12)+theme(panel.grid.minor=element_blank(),plot.title=element_text(face="bold",size=13))
ggsave("fig_s5b_monotonicity_summary.png",qA+qB+plot_layout(widths=c(1,1.3)),
