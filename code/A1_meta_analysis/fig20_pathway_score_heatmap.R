...
# Produces: fig20_pathway_score_heatmap.png
# Conda env: psoriasis-r   (run with this environment activated)
# Inputs (expected alongside / in data/): gsea_results_all.rds, dge_filt_norm.rds
# Source artifact version: 4d449fea-6226-4238-8f69-da441d0fc5f3
...
annc <- list(Group=c(NN=NNblue, PP=PPred))
scz <- t(scale(t(sc)))
ord <- order(grp, il6)
png("fig20_pathway_score_heatmap.png", width=1500, height=750, res=150)
pheatmap(scz[, ord], cluster_cols=FALSE, cluster_rows=TRUE,
         annotation_col=ann, annotation_colors=annc,
