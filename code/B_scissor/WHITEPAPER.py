...
# Produces: WHITEPAPER.html
# Conda env: python   (run with this environment activated)
# Inputs (expected alongside / in data/): none
# Source artifact version: 56c8ece6-7002-40b5-9bea-9687bab93fc8
# NOTE: absolute artifact paths rewritten to basenames; place named inputs in the same dir.
#------------------------------------------------------------

import base64, os

# figure inventory: (repo relative name, version_id, caption)
figs = {
 "composition": ("fa7106af-da2b-4f14-9a7a-bf4ab170158a","Single-cell reference composition (GSE173706, Ma et al. 2023). (A) Cells per biopsy tier after QC. (B) Cells per donor, stacked by tier; 11 donors contribute paired PN+PP biopsies."),
 "ref_umap": ("bb001486-de6c-4552-8d17-d5eca80418c9","Reference UMAP (89,058 cells). (A) Nine broad lineages by canonical markers. (B) Cells colored by tier — peri-lesional (PN) cells occupy an intermediate position between normal (NN) and lesional (PP)."),
 "alpha": ("bdf009c0-4784-44dc-9932-9bf05025c0fc","Scissor alpha tuning. Selected fraction across the graph-smoothing grid; alpha=0.40 was chosen (14.84% selected, under the 20% cutoff)."),
 "scissor_umap": ("49ec897d-f858-44f0-9b0f-ccff85cee43d","Scissor selection on the reference UMAP (20k subset). Scissor+ (gradient-tracking, lesional-associated), Scissor- (normal-associated), and background cells."),
 "scissor_comp": ("e2b856ce-3237-46cb-99a1-2267d0bfea5c","Cell-type and tier composition of the Scissor-selected fractions. Scissor+ peaks at the PN/peri-lesional tier."),
 "signif": ("2f13500a-bb38-47c7-a40d-61d79db2eb54","Significance controls. (A) Reliability test: real CV-MSE vs 100 label permutations. (B) Selection permutation null: real pos-minus-neg tier gap vs 30 shuffled-label reruns."),
 "program": ("cae36582-8fd3-4579-a08b-dc44674672e7","Gradient-tracking gene program: volcano of Scissor+ vs background differential expression (1,861 genes at padj<0.05)."),
 "stat3": ("83afab95-0678-4208-8e94-7ea7e12e4af0","STAT3 expression across Scissor classes: significantly elevated in Scissor+ (log2FC 0.43, padj 0.018)."),
 "deconv": ("5432311e-45cb-4b15-b08c-b25bb2972671","Orthogonal bulk deconvolution vs Scissor direction, by cell type. Endothelial (lesional-tracking), Fibroblast and Melanocyte (normal-tracking) are concordant; NK and Keratinocyte discordant; DC non-significant."),
}
# base64-encode each for the standalone HTML
fig_b64 = {}
for k,(vid,cap) in figs.items():
    with open("{{artifact:"+vid+"}}", "rb") as fh:
        fig_b64[k] = base64.b64encode(fh.read()).decode()
    print(k, "encoded", len(fig_b64[k])//1024, "KB")
print("all figures encoded")
...
with open("WHITEPAPER.html","w") as f:
    f.write(html)
print("wrote WHITEPAPER.html", os.path.getsize("WHITEPAPER.html")//1024, "KB")