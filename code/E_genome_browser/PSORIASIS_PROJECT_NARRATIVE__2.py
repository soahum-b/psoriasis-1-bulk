...
# Produces: PSORIASIS_PROJECT_NARRATIVE.pdf
# Conda env: python   (run with this environment activated)
# Inputs (expected alongside / in data/): stat3_headline.png
# Source artifact version: 82a953d6-caf6-40cf-8c72-17589fe2adb3
# NOTE: absolute artifact paths rewritten to basenames; place named inputs in the same dir.
#------------------------------------------------------------

import markdown, weasyprint, os

md_text = """# Psoriasis, from expression to target: the story of the project
...
fig_path = "stat3_headline.png"
fig_html = f'<figure><img src="file://{fig_path}" style="width:100%;max-width:680px;"/><figcaption>STAT3 locus browser: (a) full-locus \u03b1/\u03b2 transcript models, (b) C-terminus coding divergence (\u03b2 terminates early; \u03b1 extends ~1.1 kb further), (c) gene expression rising NN\u2192PN\u2192PP, (d) \u03b2-isoform fraction flat.</figcaption></figure>'

html_body = markdown.markdown(md_text, extensions=["tables","fenced_code","sane_lists"]).replace("<p>[[FIGURE]]</p>", fig_html)

css = """@page { size: A4; margin: 2cm 2.2cm; @bottom-center { content: counter(page); font-size:8pt; color:#888; } }
body { font-family:'Palatino','Georgia',serif; font-size:10.5pt; line-height:1.5; color:#1a1a1a; }
h1 { font-size:20pt; color:#7a1030; border-bottom:2px solid #7a1030; padding-bottom:6px; line-height:1.2; }
h2 { font-size:14pt; color:#7a1030; margin-top:1.5em; border-bottom:1px solid #ddd; padding-bottom:3px; }
em { color:#444; } table { border-collapse:collapse; width:100%; font-size:9pt; margin:1em 0; }
th,td { border:1px solid #ccc; padding:5px 8px; text-align:left; vertical-align:top; } th { background:#f4e8ec; }
code { background:#f2f2f2; padding:1px 3px; font-size:9pt; } figure { text-align:center; margin:1.2em 0; }
figcaption { font-size:8.5pt; color:#666; font-style:italic; margin-top:4px; } hr { border:none; border-top:1px solid #ddd; margin:1.5em 0; } p { text-align:justify; }"""

full = f"<html><head><meta charset='utf-8'><style>{css}</style></head><body>{html_body}</body></html>"
weasyprint.HTML(string=full, base_url=".").write_pdf("PSORIASIS_PROJECT_NARRATIVE.pdf")
print("PDF bytes:", os.path.getsize("PSORIASIS_PROJECT_NARRATIVE.pdf"))