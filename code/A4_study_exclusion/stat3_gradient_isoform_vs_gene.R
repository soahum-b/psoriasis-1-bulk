...
# Produces: stat3_gradient_isoform_vs_gene.csv
# Conda env: psoriasis-r   (run with this environment activated)
# Inputs (expected alongside / in data/): none
# Source artifact version: 087461f6-2a17-486a-bb9b-4f76963e8035
...
psd_old <- readRDS("per_study_de.rds")
...
gene_grad <- data.frame(
  ...
  stat3_logFC=c(
    meta_new$PPvsNN$logFC[meta_new$PPvsNN$gene=="STAT3"],
    meta_new$PNvsNN$logFC[meta_new$PNvsNN$gene=="STAT3"],
    meta_new$PNvsPP$logFC[meta_new$PNvsPP$gene=="STAT3"]),
  FDR=c(
    meta_new$PPvsNN$FDR[meta_new$PPvsNN$gene=="STAT3"],
    meta_new$PNvsNN$FDR[meta_new$PNvsNN$gene=="STAT3"],
    meta_new$PNvsPP$FDR[meta_new$PNvsPP$gene=="STAT3"]))

grad_tab <- data.frame(
  arm=c("NN","PN","PP"),
  PSI_beta_SRP165679_pct=c(8.85,7.69,8.78),
  STAT3_gene_log2FC_vs_NN=c(0, round(gene_grad$stat3_logFC[2],3), round(gene_grad$stat3_logFC[1],3)))
write.csv(grad_tab, "stat3_gradient_isoform_vs_gene.csv", row.names=FALSE)