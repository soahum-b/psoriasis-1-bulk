...
# Produces: stat3_isoform_mega_vs_meta.csv
# Conda env: psoriasis-r   (run with this environment activated)
# Inputs (expected alongside / in data/): none
# Source artifact version: b62ae2c4-acc3-4a41-8d1d-e3300f65ed79
...
cmp <- data.frame(
  method=c("Meta-analysis (per-study effect, DL random-effects)",
           "Meta-analysis (per-study effect, HKSJ)",
           "Mega-analysis (all 268 samples, study covariate, quasibinomial)",
           "Mega-analysis cross-check (depth-weighted linear model)"),
  estimate_pp=c(sprintf("%+.2f", new_pool$b*100), sprintf("%+.2f", new_hk$b*100),
                sprintf("%+.2f", pp_diff), sprintf("%+.2f", co_lm[1]*100)),
  p_value=c(sprintf("%.3f", new_pool$pval), sprintf("%.3f", new_hk$pval),
            sprintf("%.3f", co_m[4]), sprintf("%.3f", co_lm[4])),
  verdict=c("significant","not significant","not significant","not significant"))
write.csv(cmp, "stat3_isoform_mega_vs_meta.csv", row.names=FALSE)